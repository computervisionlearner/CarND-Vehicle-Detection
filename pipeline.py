...
#    window_list = utils.slide_window(img)
    on_windows = utils.search_windows(img, all_windows, svc, X_scaler,spatial_feat=True, hist_feat=True,hog_channel='ALL')
    
    heat_map = np.zeros(img.shape[:2])
    heat_map = utils.add_heat(heat_map,on_windows)
    heat_map_thresholded = utils.apply_threshold(heat_map,3)
